...
function getTeam($id)
{
 ...
 function acceptMatch($matchId)
  ...
  include('config_inc.php');
	...
	$result=mysql_query('UPDATE `match` SET `State` = 2 WHERE idMatch='.$matchId);
  ...
  include('config_inc.php');
	...
	$result=mysql_query("UPDATE `match` SET `Data` = '".addslashes($Data)."' WHERE idMatch=".$matchId);
