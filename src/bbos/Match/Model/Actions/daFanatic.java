 ...
 * @author root
 */
public class daFanatic extends dAction {
    ...
    protected static ImageIcon _icon = new ImageIcon(dAction.class.getResource("/resources/images/actions/fanatic.gif"));
    ...
    public daFanatic(rmiMatch model, rmiPlayer player, rmiTeam opponent, rmiTeam myTeam, Vector opponentPlayers, Vector myPlayers, boolean isChallenger) {
        super(C_FANATIC);
        _model = model;
        _attacker = player;
        _dices = 0;
    ...
    public void selectSquares() {
        try {
            if (_step == STEP_MOVE) {
                ...
                _model.resetSquareMove();
                if (_attacker.getState() == dPlayer.C_STATE_PRONE) {
                    _model.setSquareState(new dSquare(_attacker.getX(), _attacker.getY()), dSquare.C_SQUARE_MOVE);
                } else {
                    ...
                    Vector squares_m = _attacker.getFanaticSquares();
                    for (int i = 0; i < squares_m.size(); i++) {
                        dSquare s = (dSquare) squares_m.get(i);
    ...
    public void preStep() {
        try {
            this._available_move = _attacker.getMovement();
            if (_attacker.getState() == dPlayer.C_STATE_PRONE) {
    ...
    public void step(dSquare sq) {
        ...
        try {
            ...
            if (_step == STEP_MOVE) {
                ...
                int state = _model.getSquareState(sq);

                if (state == dSquare.C_SQUARE_MOVE) {

                    if (_attacker.getState() == dPlayer.C_STATE_PRONE) {
                        if ((s.getX() == _attacker.getX()) && (s.getY() == _attacker.getY())) {
                            ...
                            if (canGetUp) {
                                _attacker.setState(dPlayer.C_STATE_OK);
                        ...
                        if (_available_move > 0) {
                            ...
                            if (_additional_move > 0) {
                                _additional_move--;
                                drGoForIt gfiRoll = new drGoForIt(_attacker, _model);
                        ...
                        _attacker.isPlaying(true);
                        int direction = bbTool.getd3();
                        dSquare dest = null;
                        switch (direction) {
                            case 1:
                                if (sq.getX() == _attacker.getX()) {
                                    if (sq.getY() - _attacker.getY() == 1) {
                                        dest = new dSquare(_attacker.getX() + 1, _attacker.getY() + 1);
                                    } else {
                                        dest = new dSquare(_attacker.getX() - 1, _attacker.getY() - 1);
                                    }
                                }

                                if (sq.getY() == _attacker.getY()) {
                                    if (sq.getX() - _attacker.getY() == 1) {
                                        dest = new dSquare(_attacker.getX() + 1, _attacker.getY() - 1);
                                    } else {
                                        dest = new dSquare(_attacker.getX() - 1, _attacker.getY() + 1);
                                    }
                            ...
                            case 2:
                                if (sq.getX() == _attacker.getX()) {
                                    if (sq.getY() - _attacker.getY() == 1) {
                                        dest = new dSquare(_attacker.getX(), _attacker.getY() + 1);
                                    } else {
                                        dest = new dSquare(_attacker.getX(), _attacker.getY() - 1);
                                    }
                                }
                                ...
                                if (sq.getY() == _attacker.getY()) {
                                    if (sq.getX() - _attacker.getX() == 1) {
                                        dest = new dSquare(_attacker.getX() + 1, _attacker.getY());
                                    } else {
                                        dest = new dSquare(_attacker.getX() - 1, _attacker.getY());
                                    }
                                }
                        ...
                        state = _model.getSquareState(dest);
                        if (state == dSquare.C_SQUARE_OPPONENT_TO_BLOCK) {
                            int number = _opponent.getPlayerNumber(dest.getX(), dest.getY());
                            if (number >= 0) {
                            ...
                            _attacker.setX(dest.getX());
                            _attacker.setY(dest.getY());
                            _step = STEP_MOVE;
                        }
                    }
                }
    ...
    public static ImageIcon getBlockDiceIcon(
            int value, boolean isChallenger) {
        switch (value) {
            case 1:
